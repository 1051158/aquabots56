...
import Trilateration as trilateration
import Draw as draw
import array
import numpy as np
...
def put_in_xls(
    WiFiString, wks, x_array, y_array, z_array, tagInfo, fig, Settings, timer, ax, plt
):
    ...
    for i in range((tagInfo.max_anchors) - 1):
        ...
        if tagInfo.AD_start != tagInfo.AD_end:
            print("ID: ", ID)
            xpos = (
                Settings.num_of_send_counter
                + 2
                + 2 * int(ID)
                + (Settings.nos + 1) * int(ID)
            )
            print("xpos: ", xpos)
            ...
            wks.write(
                xl_rowcol_to_cell(
                    Settings.position_y + 2 * int(ID), Settings.num_of_send_counter
                ),
                D,
            )
        ...
        if i == 2:
            if Settings.xyzTest:
                Coordinate1, Coordinate2 = trilateration.triliterationnew3D(
                    x_array,
                    y_array,
                    z_array,
                    D_array[0],
                ...
                x, y = trilateration.getCoordinates1(
                    x_array, y_array, D_array[0], D_array[1], D_array[2]
                )
            ...
            if tagInfo.AD_start == tagInfo.AD_end:
                if Settings.xyzTest:
                    for j in range(tagInfo.max_anchors - 1):
                        wks.write(
                            xl_rowcol_to_cell(
                                (tagInfo.max_anchors + 1) * 2,
                                Settings.num_of_send_counter + 2,
                            ),
                            Coordinate1[j],
                        )
                        wks.write(
                    ...
                    wks.write(
                        xl_rowcol_to_cell(10, Settings.num_of_send_counter + 2), x
                    )
                    wks.write(
                        xl_rowcol_to_cell(11, Settings.num_of_send_counter + 2), y
                    )
            ...
            if tagInfo.AD_start != tagInfo.AD_end:
                if Settings.xyzTest:
                    for j in range(tagInfo.max_anchors - 1):
                        xpos_xyz1 = (
                            (Settings.nos + 3) * (tagInfo.max_anchors)
                            + Settings.num_of_send_counter
                            + (Settings.nos + 1) * j
                        )
                        xpos_xyz2 = (
                        ...
                        wks.write(
                            xl_rowcol_to_cell(
                                Settings.position_y,
                                (Settings.nos + 2) * (tagInfo.max_anchors)
                                + Settings.num_of_send_counter
                                + Settings.nos * j
                                + (Settings.nos + 1) * 3
                                + 3
                    ...
                    if Coordinate1[2] > 0:
                        fig, ax, plt = draw.animate_func(
                            Coordinate1, fig, ax, plt, Settings.xyzTest
                        )
                else:
                    wks.write(
                        xl_rowcol_to_cell(
                    ...
                    fig, ax, plt = draw.animate_func(
                        Coordinate2, fig, ax, plt, Settings.xyzTest
                    )
...
def first_rows_excel_multiple_AD(wks, tagInfo, Distance_array, Settings):
    ...
    wks.write(
        xl_rowcol_to_cell(0, (Settings.nos + 2) * (tagInfo.max_anchors + 1) + 1), "y1"
    )
    wks.write(
        xl_rowcol_to_cell(0, (Settings.nos + 2) * (tagInfo.max_anchors + 2) + 2), "z1"
    )
    wks.write(
        xl_rowcol_to_cell(0, (Settings.nos + 2) * (tagInfo.max_anchors + 3) + 3), "x2"
    )
    wks.write(
        xl_rowcol_to_cell(0, (Settings.nos + 2) * (tagInfo.max_anchors + 4) + 4), "y2"
    )
    wks.write(
        xl_rowcol_to_cell(0, (Settings.nos + 2) * (tagInfo.max_anchors + 5) + 5), "z2"
    )
    ...
    for k in range((tagInfo.max_anchors)):
        ...
        ):  # make sure the max anchors is correct with reality
            wks.write(
                xl_rowcol_to_cell(i + j, row),
                float(tagInfo.AD_start) + (float(tagInfo.AD_interval) * i),
            )  # write on the cells defined by y row and the i of the for loop
            for l in range(int(number_of_dcm) + 1):
                wks.write(
                    xl_rowcol_to_cell(i + j + 1, row + 1),
                    "avg_" + str(l + int(Settings.dcm)),
                )
                j += 1
        j = 0
        row += Settings.nos + 3
...
def first_rows_excel_multiple_anchor(wks, tagInfo, Distance_array, Settings):
    ...
    for k in range((tagInfo.max_anchors)):
        ...
        for l in range(int(number_of_dcm) + 1):
            wks.write(
                xl_rowcol_to_cell(2 * k + l + 1, row + 1),
                "avg_" + str(l + int(Settings.dcm)),
            )
...
def first_rows_excel(wks, tagInfo, distance_array, Settings):
    ...
    print(Settings.dci)
    row = Settings.num_of_send_counter
    number_of_intervals = (float(tagInfo.AD_end) - float(tagInfo.AD_start)) / float(
        tagInfo.AD_interval
    )
    ...
    for i in range(
        ...
        for l in range(int(number_of_dcm) + 1):
            wks.write(
                xl_rowcol_to_cell(i + j, row + 1), "avg_" + str(l + int(Settings.dcm))
            )
            j += 1
...
def make_new_wbk(give_name):
    ...
    if "/x-y_test" or "/x-y-z_test" in testName:
        ...
        directory = (
            directory + testName + modeName + anchorName + fileName
        )  # string combined
